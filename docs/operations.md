...
`radian-dev-test-map` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies test behavior through round-trip fixtures, with lossless normalization checks and fixture-scale datasets.
...
The score is deterministic. The current threshold is 157. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 3.
